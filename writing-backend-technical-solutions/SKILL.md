...
- 完整方案必须包含“扩展性与维护性设计建议”相关内容，用于说明本次设计如何降低后续变更成本、隔离业务规则变化和保持模块边界清晰。
- 扩展性与维护性建议可以包括设计模式、设计原则、职责边界、领域抽象、配置化策略、扩展点、兼容演进路径或代码组织方式，但必须基于当前代码和目标需求给出，不要泛泛罗列名词。
- 推荐设计模式时必须说明它解决的具体变化点，例如状态机/策略模式用于隔离状态流转规则，模板方法用于收敛重复处理骨架，适配器用于隔离上下游协议差异，工厂用于封装对象创建或规则选择。
- 如果当前场景不适合引入新的模式或抽象，也要给出维护性建议，例如沿用现有分层、收口规则入口、避免重复 SQL、避免循环远程调用、补齐命名和边界注释；不要为了扩展性而制造无实际收益的新层。
...
- 完整方案必须有“扩展性与维护性设计建议”相关章节或小节，推荐内容要落到本次改造涉及的模块、规则、接口、SQL、任务或上下游依赖，不能只写 SOLID、低耦合、高内聚等口号。
...
| 扩展性与维护性设计建议 | 用什么设计模式、设计原则、边界或扩展点降低后续变更成本 |
...
- 是否单独说明了扩展性与维护性设计建议，并且每条建议都能映射到真实变化点、模块边界或后续演进风险
