...
flowchart TD
    ...
    class A,B,C,D,H,I,J,K existing;
    class E,G changed;
    class F added;
```
...
完整方案里还需要把“后续怎么扩展、怎么维护”单独说清楚。这里不是堆设计模式名词，而是把模式或原则映射到真实变化点。
...
| 变化点 | 推荐设计 | 原因 | 不采用时的风险或替代方案 |
|---|---|---|---|
| 订正状态从二态扩为三态，后续可能继续增加验证类状态 | 使用 `WrongQuestionStatusManager` 收口状态机规则 | 状态变化已经影响订正、列表、统计和日清，集中计算 `nextStatus` 能避免状态判断散落在 Facade、Mapper 和 Job 中 | 如果继续在各入口写 `if correct_status`，后续新增状态时容易漏改统计或任务口径 |
| 待验证二级 TAB 规则可能继续调整 | 用策略枚举或规则方法封装 `新加入/可挑战` 判定 | TAB 规则与 SQL 查询、时间边界、分页强相关，需要有单一规则入口 | 不建议先抽复杂策略框架；当前可先用枚举 + 明确方法命名保持最小闭环 |
| 脚手架打标只更新辅助字段，不应直接改主状态 | 保持命令职责单一，打标接口和订正接口分离 | 避免前端行为打点与业务状态流转互相污染，便于后续替换脚手架来源 | 如果合并到订正接口，后续多端打标或补偿任务会更难复用 |
| 列表、统计、日清都依赖新状态口径 | 把 SQL 条件和统计口径集中到 Mapper/Repository 层命名清晰的方法 | 防止不同入口各自拼条件造成口径漂移 | 禁止在循环中逐条查询错题记录，应批量读取并在内存映射 |
| MQ 下游可能对 `QUESTION_CORRECTED` 有旧语义依赖 | 用兼容演进方式扩展消息语义或要求下游回查主表 | 避免把 `待验证` 误当成最终已订正 | 若不能确认下游语义，应标记为待确认并影响发布策略 |
```

这类章节要回答“为什么这么设计更可维护”，不要只写“遵循开闭原则”。如果没有必要引入新模式，也可以明确写“沿用现有分层 + 收口规则入口”。
...
sequenceDiagram
    ...
    alt 命中脚手架待验证规则
        STATE-->>BE: nextStatus = 待验证
        BE->>DB: 更新 correct_status 与脚手架字段
    else 保持原二态口径
        STATE-->>BE: nextStatus = 未订正 / 已订正
        BE->>DB: 按原口径更新 correct_status
    end
    opt 需要通知下游
        BE-)MQ: 发送订正消息
    end
