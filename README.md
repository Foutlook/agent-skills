...
仿照 Claude Code `/simplify` 的 cleanup-only 工作流，对当前改动、最近提交或指定文件与符号做代码简化审查。默认并行从复用、简化、效率、抽象层级四个角度检查，并直接应用证据充分、行为不变的安全清理；风险项会被跳过，但不会阻塞其他安全修改。

无参数时优先审查未暂存和已暂存 diff；工作区干净时自动审查最新提交。指定类、方法或其他符号时读取完整文件，而不是只看 diff 片段。本 Skill 不负责寻找或修复逻辑 Bug、安全漏洞和并发问题，这类问题应交给 code review、security review 或调试流程。

**适用场景**:
- 用户要求"代码简化""代码重构""清理重复代码""优化最近提交"
- 需要对当前 diff、最新提交、指定提交范围或完整文件执行 `/simplify` 风格清理
- 需要检查重复实现、不必要复杂度、明确的效率浪费和不合适的抽象层级
- 需要自动应用安全 cleanup，同时跳过可能改变行为、兼容性、数据范围或调用时机的建议
